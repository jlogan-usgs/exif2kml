...
#                coords=[(longitude, latitude)])
    photo = kml.newphotooverlay(name=os.path.basename(fname))
    photo.camera = simplekml.Camera(longitude=longitude, latitude=latitude, altitude=100,
                                altitudemode=simplekml.AltitudeMode.relativetoground)
    photo.point.coords = [(longitude, latitude)]
    photo.style.iconstyle.icon.href = 'C:\\temp\sampgtag\IMG_7919_thumb.png'
    photo.icon.href = fname
    photo.viewvolume = simplekml.ViewVolume(-60,60,-60,60,10)
